...
//wilsom	拷贝sdk/vv目录
gulp.task("copyDir_sdk_vv",["dealEngineFolder2_VIVO"],function() {
	// 如果不是vivo快游戏
	if (platform !== "vivogame") {
		return;
	}
	let dir = path.join(workSpaceDir, "/sdk/vv/**/*").replace(/\\/g, "/");
	return gulp.src(dir)
        .pipe(gulp.dest(projSrc));
})
...
gulp.task("dealNoCompile1_VIVO", ["copyDir_sdk_vv"], function() {
	// 如果不是vivo快游戏
	if (platform !== "vivogame") {
		return;
	}
...
//wilsom	代码混淆
gulp.task("encode_vv_js", ["dealNoCompile2_VIVO"], function () {
	// 如果不是vivo快游戏
	if (platform !== "vivogame") {
		return;
	}
	let engineFolder = path.join(projDir, "engine","js");
	let dir = engineFolder.replace(/\\/g, "/") + "/";
	console.log("---------------------------------------------");
	console.log(dir);
	console.log("---------------------------------------------");
   	return gulp.src([dir+"bundle.js"])
...
// 打包rpk
gulp.task("buildRPK_VIVO", ["encode_vv_js"], function() {
	// 如果不是vivo快游戏
	if (platform !== "vivogame") {
		return;
	}
