...
//wilsom begin 
//拷贝sdk/op目录
gulp.task("copyDir_sdk_op",["version_OPPO"],function() {
	// 如果不是vivo快游戏
	if (platform !== "oppogame") {
		return;
	}
...
//代码混淆
gulp.task("encode_op_js", ["copyDir_sdk_op"], function () {
	// 如果不是vivo快游戏
	if (platform !== "oppogame") {
		return;
	}
	var dir = path.join(projDir, "js").replace(/\\/g, "/") + "/";
	return gulp.src([dir + "bundle.js"]).pipe(
...
// 打包rpk
gulp.task("buildRPK_OPPO", ["encode_op_js"], function() {
	// 如果不是OPPO快游戏
	if (platform !== "oppogame") {
		return;
	}
